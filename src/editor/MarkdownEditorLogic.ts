...
export function createMarkdownEditorComponent(DCLogic, React) {
  ...
  _coreVals() {
    return {
      ...
      // 与其他菜单动作一致：执行后关闭下拉（否则菜单悬在标签栏上方挡点击）
      showEditorMode: () => { this.toggleMenubar(''); this.setViewMode('editor'); },
      showSplitMode: () => { this.toggleMenubar(''); this.setViewMode('split'); },
      showPreviewMode: () => { this.toggleMenubar(''); this.setViewMode('preview'); },
  ...
  _menuVals() {
    return {
      ...
      menuTheme: () => { this.toggleMenubar(''); this.toggleTheme(); },
      menuSettings: () => { this.toggleMenubar(''); this.openSettings(); },
      importFont: () => { this.toggleMenubar(''); this._importFont(); },
      menuUndo: () => { this.toggleMenubar(''); this.undoEdit(); },
      menuRedo: () => { this.toggleMenubar(''); this.redoEdit(); },
      // 打开搜索条是覆盖型动作：先关菜单，避免菜单浮层遮住搜索条（搜索条在源码区上方）
      menuSearch: () => { this.toggleMenubar(''); this.toggleSearch(); },
      menuH1: () => { this.toggleMenubar(''); this._linePrefix('# '); },
      menuH2: () => { this.toggleMenubar(''); this._linePrefix('## '); },
      menuH3: () => { this.toggleMenubar(''); this._linePrefix('### '); },
      menuPaperInk: () => { this.toggleMenubar(''); this.setPaper('ink'); },
      menuPaperParchment: () => { this.toggleMenubar(''); this.setPaper('parchment'); },
      menuPaperCream: () => { this.toggleMenubar(''); this.setPaper('cream'); },
