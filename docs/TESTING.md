...
| 层 | 工具 | 位置 | 跑什么 |
| --- | --- | --- | --- |
| 单元测试 | Node 内置 `node:test`（零依赖，TS 直跑） | `tests/unit/*.test.ts` | 模块级逻辑：格式化命令、撤销历史、视图状态、存储、本地文件同步 |
| 端到端 | Playwright + Chromium | `tests/e2e/*.spec.ts` | 真实浏览器中的用户链路：编辑 → 预览 → 持久化 |
...
- E2E 跑的是默认 Vite 模式（tauriBridge 为 null）。桌面端（Tauri）专属的本地文件能力由单测覆盖逻辑层，并在 `src-tauri` 侧以 Rust 集成验证。
...
- 测试目录不参与 `tsc --noEmit`（tsconfig 只含 `src`），stub 可以写得宽松。
